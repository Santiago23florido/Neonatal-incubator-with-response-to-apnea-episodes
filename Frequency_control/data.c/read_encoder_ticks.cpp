...
int main() {
    ...
    stdio_init_all();
    printf("Motor speed measurement program starting...\n");
    ...
    // Test settings
    const int test_duration_ms = 10000;  // 10 seconds per test
    const float forward_speed = 50.0f;   // 50% speed forward
    const float reverse_speed = 50.0f;  // 30% speed reverse
    
    printf("Starting continuous motor test sequence\n");
    
    // Main test loops
    bool is_forward = true;  // Start with forward direction
    uint32_t last_direction_change = to_ms_since_boot(get_absolute_time());
    uint32_t last_print_time = last_direction_change;
    
    while (true) {  // Run indefinitely
        uint32_t current_time = to_ms_since_boot(get_absolute_time());
        
        // Change direction every test_duration_ms
        if (current_time - last_direction_change >= test_duration_ms) {
            is_forward = !is_forward;  // Toggle direction
            
            if (is_forward) {
                printf("\nChanging to forward direction at %.1f%% speed\n", forward_speed);
                motor.set_motor(forward_speed);
            } else {
                printf("\nChanging to reverse direction at %.1f%% speed\n", -reverse_speed);
                motor.set_motor(reverse_speed);
            }
            
            motor.resetEncoderTicks();  // Reset encoder at direction change
            last_direction_change = current_time;
        }
        
        // Display speed information every 100ms
        if (current_time - last_print_time >= 100) {
            float revolutions = 0.0f;
            float rpm = motor.calculate_rpm(&revolutions);
            
            printf("Direction: %s, Revolutions: %.2f, RPM: %.2f\n", 
                   is_forward ? "Forward" : "Reverse", 
                   revolutions, rpm);
            
            motor.toggleLED();  // Toggle LED to indicate activity
    ...
    // We'll never reach this point due to the infinite loop
    return 0;
}